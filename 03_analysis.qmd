...
First, we consider removing 1998, 2011, 2013 as they have sampling gaps around
the peak of migration

```{r}
#| code-fold: false
m0a <- update(m0, data = filter(v1, !year %in% c(1998, 2011, 2013)))
```
...
As does the post-hoc comparison of slopes among the measures. 
...
```{r}
#| code-fold: false
m0b <- update(m0, data = filter(v1, !year %in% c(2001, 2006, 2023)))
```
...
Removing these years from the analyses, reduced the strength of the patterns such that significant effects became trends. 
However there were few changes to the magnitude of the slopes. 
```{r}
#| column: body
#| layout-nrow: 1
#| code-fold: false
...
Out of curiosity, let's try removing all 6 years. This reduces our
...
```{r}
#| code-fold: false
m0c <- update(m0, data = filter(v1, !year %in% c(1998, 2011, 2013, 
                                                 2001, 2006, 2023)))
```
...
Removing these years from the analyses, reduced the strength of the patterns in the end of the peak such that it becomes a trend, but all other measures
show little changes. Further, there are few changes to the magnitude of the slopes. 
```{r}
#| column: body
#| layout-nrow: 1
#| code-fold: false
...
the error around a slope. But the magnitudes of the slopes do not change and even
the significance is only ever reduced to trends. 
...
```{r}
#| code-fold: false
m1a <- update(m1, data = filter(v, !year %in% c(1998, 2011, 2013)))
m2a <- update(m2, data = filter(v, !year %in% c(1998, 2011, 2013)))
```
...
```{r}
#| code-fold: false
m1b <- update(m1, data = filter(v, !year %in% c(2001, 2006, 2023)))
m2b <- update(m2, data = filter(v, !year %in% c(2001, 2006, 2023)))
```
...
```{r}
#| code-fold: false
m1c <- update(m1, data = filter(v, !year %in% c(1998, 2011, 2013, 
                                                2001, 2006, 2023)))
m2c <- update(m2, data = filter(v, !year %in% c(1998, 2011, 2013, 
                                                2001, 2006, 2023)))
```
...
For skew of the counts during the migration period, if we omit 2001, the patterns
don't really change (i.e., stay non-significant).
```{r}
compare(m1, 2002)
```

For skew of the counts during the peak migration period, if we omit 2002, the patterns
don't really change (i.e., stay non-significant).
```{r}
compare(m2, 2002)
```

For kurtosis during the peak migration period, if we omit 2002, the patterns don't really 
change. 
```{r}
compare(m5, 2002)
```
...
For kurtosis during the migration period, if we omit 2015, the patterns are stronger, 
but the same.
```{r}
compare(m4, 2015)
```
