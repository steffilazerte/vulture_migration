...
# Problematic years
v <- mutate(v, gaps = if_else(year %in% c(2011, 2013), "Gap around peak", "No gaps"))
y1 <- c(2011, 2013)         # Gaps around the peak
y2 <- c(2001, 2006, 2023)   # Missing end of migration
```
...
g1 <- ggplot(v, aes(x = year, y = date)) +
  ...
  theme(axis.title.x = element_blank(),
        legend.direction = "vertical", legend.position = "inside",
        ...
        plot.margin = margin(1,2,0,1, "pt"), panel.spacing = unit(0, "pt")) +
  geom_point(aes(shape = gaps)) +
...
g3 <- ggplot(v, aes(x = year, y = mig_raw_max)) +
  theme_bw() +
  theme(legend.direction = "horizontal", legend.position = "inside",
        ...
        plot.margin = margin(1,2,0,1, "pt"), panel.spacing = unit(0, "pt")) +
  geom_point() +
  stat_smooth(method = MASS::glm.nb, aes(fill = "All years"), alpha = 0.15) +
  stat_smooth(data = filter(v, year != 2022), aes(fill = "Without 2022"), 
              method = MASS::glm.nb, linewidth = 1, colour = "black") +
  labs(y = "Annual Maximum\nDaily Estimated Total", x = "Year") +
  scale_fill_manual(name = NULL, 
                    values = c("All years" = "blue", "Without 2022" = "darkgrey"))
...
#| fig-width: 5
#| out-width: 80%
g & theme(legend.position.inside = c(0.17, 0.85))
gg & theme(legend.position.inside = c(0.17, 0.85))
```

### Supplemental comparison of with/without 2022
