...
**The median number of residents for 1998 is `r cum_counts$res_pop_median[1]`**
...
```{r}
skew_mig <- dts |>
  filter(perc %in% c("p05", "p95")) |>
  select(year, perc, doy_passage) |>
  pivot_wider(names_from = "perc", values_from = "doy_passage") |>
  left_join(x = pred, y = _, by = "year") |>
  filter(doy >= p05 & doy <= p95) |>
  group_by(year) |>
  summarize(mig_skew = skewness(count),
            mig_kurt = kurtosis(count))

skew_peak <- dts |>
  filter(perc %in% c("p25", "p75")) |>
  select(year, perc, doy_passage) |>
  pivot_wider(names_from = "perc", values_from = "doy_passage") |>
  left_join(x = pred, y = _, by = "year") |>
  filter(doy >= p25 & doy <= p75) |>
  group_by(year) |>
  summarize(peak_skew = skewness(count),
            peak_kurt = kurtosis(count))
```
